// js/ui.js
// Componentes de UI reutilizados entre páginas: navbar, cards, toast, modal.
...
/**
 * Monta a navbar padrão em qualquer página. `activePage` destaca o link atual.
 */
export function renderNavbar(container, { activePage, modoAtivo, perfilLogado, onModoChange }) {
  ...
  container.innerHTML = `
    <div class="navbar-inner">
      ...
      <div class="mode-switch" aria-label="Modo de visualização">
  ...
  container.querySelectorAll('[data-mode-btn]').forEach(btn => {
    ...
    btn.addEventListener('click', () => onModoChange && onModoChange(btn.dataset.modeBtn));
  });
...
/**
 * Renderiza um card de título dentro do grid.
 */
export function renderTituloCard(titulo, modo) {
  const notaPrincipal = obterNotaExibicao(titulo, modo);
  const capa = titulo.capa_url || placeholderCapa();
  const statusChip = renderStatusChip(titulo.status);
  ...
  card.innerHTML = `
    <div class="poster-wrap">
      ...
      ${titulo.pendente ? '<span class="badge-pending">Pendente</span>' : ''}
    </div>
    <div class="card-body">
      <div class="card-title">${escapeHtml(titulo.nome)}</div>
      <div class="card-meta">${titulo.ano || '—'}${titulo.generos && titulo.generos.length ? ' · ' + escapeHtml(titulo.generos.slice(0, 2).join(', ')) : ''}</div>
